...
type Props = {
  selector: string
  onInject: (selector: string) => void
}

const RedirectUriInjector = ({ selector, onInject }: Props) => {
  ...
  return (
    <span className="suggestion-anchor">
      <span
        ...
        title="Input redirect_uri to page"
      >
      ...
      {open && (
        <div ref={popoverRef} className="suggestion-popover">
          <pre
            ...
            onClick={() => { onInject(selector); setOpen(false); }}
            title="Click to input redirect_uri to page"
          >Input redirect_uri to page</pre>
        </div>
      )}
    </span>
  );
...
export default RedirectUriInjector;
